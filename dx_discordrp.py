...
def get_shadps4_path():
    os_system = platform.system()
    default_data_path = None

    if os_system == 'Darwin':  # macOS
        default_data_path = Path.home() / "Library/Application Support/shadps4"
    elif os_system == 'Windows':
        default_data_path = None
    elif os_system == 'Linux':
        default_data_paths = [
            Path.home() / ".config/shadps4",
            Path.home() / ".shadps4",
            Path("/usr/share/shadps4"),
            Path("/usr/local/share/shadps4")
        ]
    ...
    while True:
        if default_data_path and default_data_path.exists():
            print(f"Default ShadPS4 data directory detected: {default_data_path}")
            print(f"ShadPS4 Directory must contain 'data' folder.\nWhere Rock Band 4 Deluxe is installed in /data/CUSA02084/GoldHEN/RB4DX-1.08/.")
            shadps4_path_str = input(f"Enter the path for ShadPS4 data directory (leave empty to use default): ").strip()
            if not shadps4_path_str:
                shadps4_path = default_data_path
            else:
                shadps4_path = Path(shadps4_path_str)
        else:
            print(f"ShadPS4 Directory must contain 'data' folder.\nWhere Rock Band 4 Deluxe is installed in /data/CUSA02084/GoldHEN/RB4DX-1.08/.")
            shadps4_path_str = input("Enter the path for ShadPS4 base directory (e.g. C:\\games\\shadps4): ").strip()
            shadps4_path = Path(shadps4_path_str)

        if shadps4_path.exists() and shadps4_path.is_dir():
            return shadps4_path
        else:
...
def main():
    ...
    large_text = "Rock Band 4 Deluxe"  # Default value for large_text
